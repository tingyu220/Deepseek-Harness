...
export function spawnSubprocess(spec: SubprocessSpawnSpec, internals: SpawnInternals = {}): LocalSubprocessHandle {
  ...
  const child = spawn(program, args, {
    ...
    // The desktop shell must never flash a console window for a command the
    // GUI is already rendering (skill install, bash/pwsh tools, ...); the
    // option is ignored on non-Windows hosts.
    windowsHide: true,
  })
